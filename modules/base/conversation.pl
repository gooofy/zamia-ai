...
nlp_sayagain_s (en, S) :- hears(en, S, "huh?").
% nlp_sayagain_s (en, S) :- hears(en, S, "say again please?").
nlp_sayagain_s (en, S) :- hears(en, S, "say again?").
nlp_sayagain_s (en, S) :- hears(en, S, "what was that?").

nlp_isaid_r (en, R) :- says (en, R, "I said").
%nlp_isaid_r (en, R) :- says (en, R, "As I just said").
%nlp_isaid_r (en, R) :- says (en, R, "I repeat ").

%
% very basic building blocks for topic handling
%

nlp_whatwasourtopic_s (en, S) :- hears(en, S, "what was our topic?").
nlp_whatwasourtopic_s (en, S) :- hears(en, S, "what did we talk about?").
nlp_whatwasourtopic_s (en, S) :- hears(en, S, "sorry I lost my train of though?").
nlp_whatwasourtopic_s (en, S) :- hears(en, S, "where were we?").
