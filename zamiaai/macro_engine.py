...
class MacroEngine(object):
    ...
    def load (self, module_name):
        ...
        fn = 'modules/%s/named_macros.json' % module_name
    ...
    def add_named_macro(self, module_name, lang, name, soln):
        ...
        if not module_name in self.named_macros_mod:
            self.named_macros_mod[module_name] = {}
        if not lang in self.named_macros_mod[module_name]:
            self.named_macros_mod[module_name][lang] = {}
        if not name in self.named_macros_mod[module_name][lang]:
            self.named_macros_mod[module_name][lang][name] = []

        self.named_macros_mod[module_name][lang][name].append(soln)
